...
a per-model figure for how much context that model reads *well*, and a way to measure a prompt against it
without leaving the box.

The figure is deliberately the model's *optimal* context, not its advertised maximum.
Long-context benchmarking (NVIDIA's RULER and its successors) finds a model reliably uses only half to
two-thirds of the window it advertises: past that its recall frays — facts in the middle get lost, details
get invented — with no error to show for it. So the number held here is the size past which quality quietly
degrades, not the size the model will accept before it hard-truncates.
...
and every use keeps a margin (config.CONTEXT_SAFETY_MARGIN) for that approximation and for the reply's own
output tokens. If tiktoken can't load its encoding at all (an offline box with nothing cached), counting
falls back to a character estimate pitched high on purpose, so the fallback never under-counts and waves an
...
# tiktoken carries no qwen encoding; o200k_base is a modern BPE whose token counts sit close enough to serve
# as a conservative estimate, which is all the budget guard needs — a bound, not an exact count.
...
# qwen3.5:4b advertises ~262K tokens; the reliable window is nearer half that, so 131072 (128K) is its optimal.
# nomic-embed-text is the embedder, not a generative model — it caps its own input via num_ctx (embedding.py),
...
def count_tokens(text: str) -> int:
    ...
    The count is an estimate either way — tiktoken's o200k_base is not qwen's tokeniser — which is why every
    caller holds it against a budget kept a margin below the true optimal, never against the optimal itself.
    """
...
def truncate_tokens(text: str, max_tokens: int) -> str:
    ...
    Truncates on the same measure count_tokens uses — real tokens when tiktoken is loaded, characters when
    it isn't — so the cut agrees with the count that decided it was needed.
    """
    if max_tokens <= 0:
        return ""
    encoding = _get_encoding()
