...
A missive owes nothing to a prior /intake — it's a nudge, or a line relayed from the World,
that the kernel authored and addressed to a symbiot.
It lives apart from intake because it doesn't share intake's shape:
an intake row is a question walking toward an answer,
and a missive has neither a question nor a walk — the kernel wrote it, it *is*
the content, and there's nothing to compute. So there's no state machine here,
only a record, a read, and an acknowledgement.

The shell never sent a missive, so it holds no id to ask about one —
...
# the three pure data-access primitives first,
# in the arc the module docstring narrates —
# raise_for (the record), unseen_for_symbiot (the read), mark_seen (the acknowledgement) —
# and then deliver, which sits one layer up
# and composes them into the kernel's act of reaching out.
# The order mirrors the "record, read, acknowledgement … deliver one layer up"
...
def raise_for(conn, symbiot_id: int, body: str) -> int:
    ...
    Persisted unseen (seen_at null), which is what /inbox reads.
    There's no work to walk toward — a missive is the message, not a question —
    so it's ready the instant it's written;
    nothing moves it forward, because there's nowhere forward to move.
    """
    row = conn.execute(
...
def mark_seen(conn, symbiot_id: int, ids: list[int]) -> int:
    ...
    Scoped to the caller's own still-unseen missives:
    an id that isn't theirs or is already seen changes nothing —
    so acking is safe, idempotent,
    and can't be turned into a way to touch another symbiot's missives.
    """
    if not ids:
        return 0
    cursor = conn.execute(
...
def deliver(pool, symbiot_id: int, body: str) -> int:
    ...
    The whole act of the kernel reaching out on its own,
    over two channels so first contact never rides on one holding up:
      • the record — raise_for writes the missive durably,
        so /inbox surfaces it on the next open no matter what.
        This is the guarantee; it always lands.
      • the nudge — a best-effort notification (notify.dispatch)
        so it arrives promptly rather than only on next open.
        This is the speed; it may be skipped
        (a channel off, no channel, a dead one)
        without weakening the guarantee, because the record already stands.
    Takes a pool, not a connection: the write is its own quick transaction,
    and the notification runs outside any transaction
    (it's slow external I/O and must not hold one open).
    """
