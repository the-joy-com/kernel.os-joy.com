...
A session lives for [`config.SESSION_TTL_SECONDS`](../core/config.py), **five days** — long enough that the shell needn't re-ask for a login on every reload, short enough that a forgotten open tab doesn't stay authed indefinitely. It is a code constant rather than an environment override, unlike the abuse limits: the two ends of that trade are a judgment about the shell's feel, not a knob a box should want to turn. [`/logout`](../main.py) revokes it by stamping `revoked_at`, and is idempotent: no token, or an already-revoked one, is a clean no-op.
