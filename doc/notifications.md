...
A symbiot can be signed in from more than one browser at a time — a laptop and a phone both open, both authenticated as the same identity. The three things above behave differently under that, and the difference is worth spelling out rather than leaving to be inferred from the mechanism.

A **reply** to a line the symbiot just sent stays scoped to the device that sent it, but as a side effect of how it is addressed rather than any explicit multi-device rule: `/intake` hands back the row's own id, and only the browser that asked keeps that id and polls `/answers` for it. A second device that never asked never holds the id, so it never fetches or shows that reply — a correlation handle, not a device check, is what keeps the two apart.

A **missive** is scoped to the symbiot rather than to any one device, since it is a reach the kernel raised on its own rather than an answer to a particular question — and that is where the story gets less clean. Every signed-in device polls `/inbox` on its own timer, and there is no lock between the unseen read and the acknowledgement (`unseen_for_symbiot` / `mark_seen`, [`missive.py`](../services/loop/missive.py)): two devices polling inside the same narrow window can both see the same unseen missive before either has acked it, and both will show it. So the guarantee is narrower than exactly-once, and the module's header, the `/inbox` route and the table's own schema comment all state it that way: a missive keeps being offered until some device acknowledges it, and never after — which holds across opens, and across devices once one of them has acked first. Nothing today guards the window before that first ack. It costs a duplicate on screen and nothing more: one missive is one row, written once and read by both, so a second device showing it spends no extra work and no model call.

A **notification**, by contrast, is deliberately not scoped to a single device: fan-out reaches every subscription and address the symbiot's identity carries, precisely so a laptop and a phone both hear about the same missive through email or web push regardless of which one happens to be open when it fires. The one place the two devices reach back into each other is presence — `suppress_when_present` reads a single symbiot-level `last_seen_at`, not a per-device one, so activity on one device can suppress a push meant to reach the other.
