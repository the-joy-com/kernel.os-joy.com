...
One thing this design does **not** do, and it governs everything below: it *extends* the shipped read path, it does not replace any of it. The long-term diary reach ([`retrieval.search`](../services/memory/retrieval.py)), the reply composition ([`reply.compose`](../services/loop/reply.py)), live ingestion, and the budget backstop ([`llm._fit`](../services/adapters/llm.py)) are all built and proven — how the diary is searched, ranked, filed, and folded into the prompt does not change. Short-term memory is added *beside* long-term memory: a second kind of recall, answering "what were we just saying?" by recency where the diary answers "what do I know that bears on this?" by relevance. The one resource the two share is the prompt's token window; the sections below say how each is kept within its share — the diary by a post-hoc condense on overrun, the conversation by a background fold that trims its tail — rather than by a hard cap on either read.
...
flowchart TD
    ...
    subgraph worker["worker thread — services/loop/worker.py"]
    ...
    subgraph reply["compose the reply — services/loop/reply.py"]
        compose["_compose_prompt<br/>persona<br/>+ diary facts (long-term)<br/>+ Gist then verbatim tail (short-term)<br/>+ the current message"] --> gen["llm.generate_json(role: reply)<br/>_SpokenReply"]
    end
...
### the two clocks: when a fact happened, and when it was told

The "effective time" that breaks those ties is not a single stamp. Every row in `diary_facts` carries **two clocks**, and the gap between them is why "effective" is the right word rather than "created". One is `created_at` — **when the fact was told**, the moment the message that filed it landed, set by the database and never null. The other is `happened_at` — **when the thing the fact describes actually happened**, which is often a different moment entirely: a symbiot files "I quit my job last spring" today, so the telling is today while the event is months back. Ordering the diary by when it was *told* would put that fact next to this morning's, when its event belongs a season ago.

`happened_at` is nullable on purpose, because not every fact names a moment. It is read out of the raw fact text once, at ingestion, by a small LLM call ([`extract_happened_at`](../services/memory/ontology.py)) that resolves relative cues — "yesterday", "last spring" — against the clock at the time of filing, and returns nothing when the fact fixes no time to anything ("I prefer tea to coffee" happens at no particular instant). When it names a time, [`ontology.persist`](../services/memory/ontology.py) writes it beside the fact; when it names none, the column stays null. Time is the one particular the write path lifts out of the free text into a fixed schema column of its own — everything else the fact says stays in its own words — because *when* a thing happened is what the read path most often needs to sort and reason over, and re-parsing it out of prose at every read would be a probabilistic guess where the schema can hold the fact exactly.

The read then **collapses the two into one effective time**: `COALESCE(happened_at, created_at)`. A fact that named its own moment is ordered and dated by that moment; a fact that named none falls back to when it was told — the best available answer to "when". This single expression is what [`retrieval.search`](../services/memory/retrieval.py) and the deep reach ([`deep_retrieval.py`](../services/memory/deep_retrieval.py)) both select as `effective_at`, what breaks the ties between equally-relevant facts described just above, and what [`reply.compose`](../services/loop/reply.py) renders as each fact's leading date (see "how it is fed" below). The read path never reads `happened_at` alone — always the collapse — so a null is invisible downstream: it simply reads as the telling time. And because the ordering leans on it, the effective time is indexed directly, the `COALESCE` and all ([`0012_read_path_indexes.sql`](../migrations/0012_read_path_indexes.sql)), so the sort is served by the index rather than computed per row. The two clocks stay apart in storage — `happened_at` added beside the existing `created_at` in [`0011_diary_fact_times.sql`](../migrations/0011_diary_fact_times.sql) — and meet only at read time: the write path records two independent facts without forcing either to stand in for the other, and leaves the collapse to the reader, who can always fall one clock back to the other but could never split a single blended stamp back into two.
...
There is no running sum and no cap on this read: the size budget does not live here. It lives entirely in the **background fold** below, as its *trigger* — the threshold at which the sweep decides the tail has grown large enough to trim, by folding its oldest turns into the Gist and advancing the cutoff. So the budget still keeps the tail near a target size in steady state, but it does it by moving the cutoff forward (which this read then honours automatically), never by hiding turns from the read. That budget is a **reserved share** of the active model's optimal context window ([`services/adapters/models.py`](../services/adapters/models.py)) — the size it reads *well* — held as a per-model constant so it travels with the model if a fallback switches tiers; in practice the reply model and the tiers it falls through share one optimal window (131072), so the figure is stable across a fallback.

The consequence of an asynchronous fold is not a blind spot but a **temporarily fatter tail**: if the sweep lags, this read simply returns a few more verbatim turns than the budget nominally reserves, which the model still reads (its optimal window sits well above the reserved slice, and its advertised maximum above that again). And should a fold lag so far that the whole prompt would overrun the window, the post-hoc budget backstop [`llm._fit`](../services/adapters/llm.py) catches it — under a single rule that governs the whole prompt (see "how it is fed"): the persona, the instructions, and the live message are sacred and never shrunk, while everything the reply *remembers* — the diary facts and the conversation together — is one compressible block the guard may condense to fit. So an overrun degrades by squeezing what is remembered, never by dropping who is speaking or what was just said, and the reply always composes. Fatter, then; never blind, and never a hard failure.
...
The fold is done by a background sweep — a fourth loop beside the worker pool, the reconcile sweep, and the ingestion sweep, cut from the same cloth ([`worker.py`](../services/loop/worker.py)). It never runs on the path the symbiot waits on, so the reply carries zero latency from it. Each pass:
...
3. asks the model assigned the **`conversation_compress`** role — its own role, seeded from whatever the reply's is, so the fold rides the heavy model by default and an operator can split the two through `/models` — to merge the existing Gist and those older turns into **one fresh paragraph aimed at a fixed token target**, keeping the concrete facts and dropping the redundancy — the fold runs off the critical path, but the Gist is what a later reply reads back through once a turn has aged out of the tail, so its quality is load-bearing and a cheap local model proved too unreliable for it in practice. The merge crosses the model boundary as a **validated schema** ([`conversation._FoldReply`](../services/memory/conversation.py) through [`llm.generate_json`](../services/adapters/llm.py)), not free text: the summary is the model's only emittable field, so conversational filler has nowhere to land — see "why the Gist stays clean" below;
...
The whole prompt is handed to [`llm.generate_json`](../services/adapters/llm.py) with the model assigned the **`reply`** role — resolved from the store at call time ([`models.role_name`](../services/adapters/models.py)) rather than read from a config constant, so an operator's reassignment through `/models` takes effect without a code change; `config.REPLY_MODEL` is only the default that seeds that assignment on a fresh box — and taken back as a validated [`reply._SpokenReply`](../services/loop/reply.py) — one `reply` field holding the words to say, for the same reason the fold carries one: what comes back is spoken to the symbiot verbatim, so an "Of course! Here's my reply:" opener or a ``` fence has no field to land in rather than being asked not to appear. That runs under **one rule for what may be squeezed if the prompt would overrun the model's window** (the budget backstop, [`llm._fit`](../services/adapters/llm.py)): the **persona, the instructions, and the current message are sacred** — never condensed, because they are who is speaking, how to answer, and what was just said — and the **one remembered block (diary + Gist + verbatim tail) is the sole compressible region**. So an overrun is met by condensing what the reply remembers, never by dropping the sacred parts, and the reply always composes rather than failing for want of room. In the common case the prompt sits far under the window and nothing is condensed at all; in steady state the diary is small and the fold keeps the conversation near its slice, so the backstop is exactly that — a backstop, not a routine step.
...
Everything above is the fast reply — the answer the symbiot waits on, kept lexical and stateless-of-semantics so it is never slow. Tier 2 is the other half of the read path, and it never runs on that waiting path. Once a message's fast reply is settled — and the conversation around it has gone quiet for a settling lull ([`config.ENRICH_SETTLE_SECONDS`](../core/config.py)) — a background sweep ([`worker.run_enrichment_sweep`](../services/loop/worker.py)) reaches into the diary again for the whole **burst** of messages that lull closed, this time by **meaning** rather than by shared words, and, only when that reach turns up something the fast answers missed, the machine sends a single **enriched follow-up** for the burst. The fast reply is untouched: this is added beside it, the way short-term memory was added beside long-term, and it answers a third question again — not "what bears on this by relevance?" (the diary) nor "what were we just saying?" (the conversation), but "now that I have had a moment to think harder, is there something worth adding?"

### the deep reach: vector recall, then the ontology walk

The reach lives in [`deep_retrieval.py`](../services/memory/deep_retrieval.py), deliberately apart from the lexical librarian it complements, and runs in two read-only movements.

- **Vector recall** ([`recall_facts`](../services/memory/deep_retrieval.py)) — the message is embedded as a query and the nearest diary facts are pulled by cosine distance through the same HNSW the store built for it, read through the `active_diary_fact_embedding` view so a model swap costs it nothing. A fact surfaces here because it *means* something close to the message, even when it shares not one word — the reach the lexical pass, matching words, structurally cannot make. It is the mirror of the router's [`recall_candidates`](../services/memory/ontology.py) pointed the other way: the router searches ontology vectors to *classify a fact being filed*; this searches diary-fact vectors to *fetch filed facts that bear on a question*.
- **The ontology walk** ([`expand_by_concept`](../services/memory/deep_retrieval.py)) — the concepts the recalled facts are filed under are read from `diary_fact_ontology`, and the *sibling* facts filed under the same concepts are pulled alongside, most-shared-concepts first. So the reach follows the vocabulary tree the write path grew, not raw distance alone.

Both movements only ever read. The walk never routes the message or mints a type — coining vocabulary from a read pass is the write path's job ([`mint`](../services/memory/ontology.py)), and letting a question grow the diary's tree would corrupt the store slowly. [`deep_search`](../services/memory/deep_retrieval.py) joins the two — recalled facts first, in distance order, then the walked-in siblings — and drops the facts the burst's own messages became, so a burst never enriches itself. An empty result is the signal to send no enrichment at all.

### the origin reference: three legs, so a late follow-up situates itself

Because the pass lands after the fast reply, and the conversation may have moved on since, the composing call is handed the whole *origin reference* ([`enrichment.origin_reference`](../services/memory/enrichment.py)) — three things: **(a)** the burst's messages that prompted the fast answers, **(b)** the fast answers themselves, and **(c)** the turns exchanged since, read live from the conversation stream with the burst's own turns excluded. Leg (b) is load-bearing twice: it is how the gate tells new ground from ground already covered, and — named explicitly in the prompt and forbidden to be repeated — it is what keeps the follow-up from paraphrasing the answers already given. Leg (c) lets a delayed follow-up read as caught up rather than arriving out of order, and it carries any follow-up already sent on this ground, so the gate can see what it has said deeply and not say it again. This is the third kind of memory the fast path's opening note set aside: not recall by relevance, not recall by recency, but the anchor a late enrichment carries so it can place itself against everything said since it was set in motion.

### the gate that mostly stays silent

One structured call ([`enrichment.compose`](../services/memory/enrichment.py), on the model assigned the **`enrich`** role, seeded from the reply's so it is the same heavy model by default) both **gates and composes**: given the persona, the origin reference, and the deep facts, it returns whether to surface at all and, if so, the follow-up's words — a validated [`_EnrichmentReply`](../services/memory/enrichment.py) through [`llm.generate_json`](../services/adapters/llm.py), so the decision and the prose come back checked, the same strict-schema-as-grammar the router leans on. When the deep reach found nothing, the call is skipped entirely — no new ground to weigh, so no metered call spent. The bar to surface is high on purpose: the machine already answered, and a second unprompted message must earn its interruption, so **silence is the common outcome** and a suppressed pass simply records itself as considered. A follow-up that clears the bar is delivered as a **missive** ([`missive`](../services/loop/missive.py)) — the kernel reaching out on its own, discovered through the inbox on its own terms — rather than pretending to be an inline reply to a message the conversation has already left behind; it is mirrored onto the conversation stream, so the next turn remembers it too.

### not repeating itself: the lull upstream, the echo guard downstream

The follow-ups once came back *more or less the same* as one another — the same thought in slightly different clothes — because a rapid back-and-forth fired a deep pass per message, each reaching into the same patch of the diary. The fix works from both ends. **Upstream**, the pass no longer fires per message: a message becomes eligible only once the conversation has gone quiet for the settling lull ([`config.ENRICH_SETTLE_SECONDS`](../core/config.py)), and then the whole *burst* it ended — the run of messages with no gap longer than the lull — is enriched as one unit, one deep reach and at most one follow-up. So an active exchange spawns one deep pass, not one per line, and the follow-up never interrupts while the talk is still live. **Downstream** sits the guarantee the model's own no-repeat instruction cannot make: a composed follow-up is measured — by the same cosine closeness the [`/observe` echoes lens](observability.md) reads, the shared measure that now lives in [`services/echo.py`](../services/echo.py) — against every deep reply ever sent this symbiot, and one near-identical to any of them is held back whatever its age ([`enrichment.is_echo_of_prior`](../services/memory/enrichment.py)). The check is all-time because the redundancy is durable and deep replies are rare; it fails open, never suppressing when it cannot measure.

### exactly-once, the same way everything else is

A provenance table (`enrichment`, migration 0015) carries one row per message the pass has *considered*, surfaced or not, under a `UNIQUE` link back to the message; a whole burst is recorded together, a row for every member — the anchor (the burst's last message) carrying the missive if one surfaced, the rest recorded as suppressed — so none is left eligible to fire on its own. The sweep's eligibility ([`enrichment.next_burst_to_enrich`](../services/memory/enrichment.py)) is the mirror of it — the oldest *settled* burst of **answered** (there must be an answer *to* enrich, narrower than ingestion's terminal set), authed messages with no row yet. A crash mid-pass leaves no rows and the burst eligible again; a committed pass records its verdict and is never re-run. Because the whole pass reads state before it writes and spends seconds doing so, it carries the same duplicate-work race the compression fold does — two sweeps could both run the deep reach for one symbiot — and takes the same cure: a non-blocking transaction-scoped advisory lock in its own namespace ([`enrichment.claim`](../services/memory/enrichment.py)), keyed on the symbiot and claimed before the heavy work, so a second worker finds it held and skips. The follow-up and the rows that record it commit together, so a burst is sent-and-recorded atomically or neither.

```mermaid
flowchart TD
    settled["a burst gone quiet<br/>(answered messages, none newer within the lull)"] --> claim

    subgraph sweep["enrichment sweep — worker.run_enrichment_sweep (off the critical path)"]
        claim["claim<br/>(pg_try_advisory_xact_lock on the symbiot — skip if held)"] --> reach
        reach["deep_retrieval.deep_search(burst messages)"] --> gate
    end
    ...
    origin["origin reference<br/>(a) burst messages + (b) fast answers + (c) turns since"] --> gate
    gate["enrichment.compose<br/>gate-and-compose (role: enrich)"] -->|"surface = false"| silent(["record considered, send nothing"])
    gate -->|"surface = true"| echo{"is_echo_of_prior?<br/>(all-time cosine, services/echo.py)"}
    echo -->|"echoes one already sent"| silent
    echo -->|"genuinely new"| send["missive.raise_for + record_utterance"]
    send --> record["record_burst<br/>(a row per member; anchor holds the missive)"]
    silent --> record
