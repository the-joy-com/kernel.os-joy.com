"""Offline duplicate GC: the merge pass that collapses semantic duplicates, with the models faked.
...
def test_confirm_same_kind_reads_both_definitions(monkeypatch):
    ...
    # think=False, decode grammar sent — and the boolean is parsed and checked on the way back.
    captured = {}

    def fake_post(url, json, timeout):
        captured["json"] = json
        return _FakeResponse({"response": '{"same": true}'})
    ...
    assert captured["json"]["format"] == ontology_gc._SameKindReply.model_json_schema()
...
def test_pick_survivor_offers_the_cluster_names_and_returns_the_id(monkeypatch):
    ...
    def fake_post(url, json, timeout):
        captured["json"] = json
        return _FakeResponse({"response": '{"survivor": "workout_action"}'})
...
# --- the whole pass, end to end, with the models faked -------------------------------------
...
def _fake_confirm_and_pick(monkeypatch, *, same: bool, survivor: str):
    ...
    def dispatch(url, json, timeout):
        if "survivor" in json["prompt"]:
            return _FakeResponse({"response": '{"survivor": "%s"}' % survivor})
        return _FakeResponse({"response": '{"same": %s}' % ("true" if same else "false")})
...
def test_run_once_on_empty_store_does_nothing_and_calls_no_model(client, monkeypatch):
    ...
    def boom(url, json, timeout):  # pragma: no cover - must never be reached
        raise AssertionError("run_once must not call the model when there are no candidate pairs")
