...
| `POST /intake` | Takes one line off the shell's prompt — body `{ "line": "<text>" }` — and acknowledges it with `{ "msg": "roger", "data": null }`. `"roger"` means *received*, not *stored*: the line is dropped (holding it in the buffer is a separate concern that layers on top of this round trip). The `line` is required and non-empty; an empty one is a `422`. It is **deliberately uncapped in length** — the symbiot may hand over as much as they mean to in one message, a long paste is a real message rather than an abuse to guard against, and the message column is `TEXT` with no ceiling of its own, so a cap here would have invented a boundary rather than protected one. The request shape is validated by the `IntakeRequest` DTO in [`dtos.py`](./core/dtos.py). |
| `GET /answers?id=<id>` | The read half of that round trip. `/intake` hands back an id, not a reply — the answer is composed afterward by a worker — so the shell keeps the id and asks here whether it has settled and which way. Four words it knows how to act on: `answer` (done, with the reply in `data.answer`), `abandoned` (the kernel spent its retry budget and says so rather than going silent), `wait out` (still in flight — ask again), `unknown` (no message carries that id). The kernel's own state machine has more states than these; the in-flight ones collapse to the single "wait out", because settled-or-not is all the shell needs. Unauthed — the id **is** the capability — and the line the symbiot sent is never echoed back, so an answer crosses the wire as itself. A failed message's stored traceback never crosses at all: it stays the kernel's diagnostic. |
| `POST /answers/delivered` | The receipt on the way back, mirroring the outbox's on the way in. After the shell renders an answer it POSTs the id here and the kernel stamps `delivered_at` — so `answered` means the reply was produced and `delivered_at` means it actually reached the symbiot, never a hopeful guess. Only touches a terminal, still-undelivered row, so a stray, in-flight, or already-stamped id is a clean no-op. |
| `GET /inbox` | The messages the shell could not have discovered on its own: a nudge, a reminder firing, a line relayed from the World — raised by the kernel, so there was never an intake id to keep. Identity-gated, but a caller with no session gets an **empty list** rather than an error, so it is no oracle about who is registered or what is waiting. This poll is also the symbiot's presence heartbeat — it fires while the tab is visible, so the dispatcher can hold an out-of-app nudge back when the record is already in front of them. |
| `POST /inbox/seen` | Acknowledges the ids the shell has shown, so `/inbox` stops offering them. Scoped to the caller's own missives, so an id that isn't theirs changes nothing; no session, no ids, or ids already seen are all clean no-ops. |
| `GET /push/key` | The public application server key a browser subscribes with. **Null** when push is unconfigured — the shell reads that as "no push here" and falls back to poll-on-open, so a kernel with no VAPID key still serves answers, just without the nudge. |
| `POST /push/subscribe` | Registers (or refreshes) a browser's push address as a reply channel and hands back its id, which the shell threads through `/intake` so the kernel knows which channel to nudge when that message settles. Ungated like `/intake` — the right to be reachable is never fenced behind identity, and a planted push address gains an attacker nothing. A session, when present, ties the channel to that symbiot so the kernel can also reach them for a missive; without one it stays anonymous and still serves per-message reply nudges. |
...
| `GET /timezone` | Authed. The zone already stored, so the `/timezone` command opens on the current value ("your local time is currently Europe/Paris") rather than prompting into the void. The column defaults to `UTC`, so this never comes back empty. Authed for the same reason the write is: a timezone belongs to a particular symbiot, so there is no anonymous version to serve. |
| `POST /timezone` | Authed. Body `{ "location": "<a place, in the symbiot's own words>" }`. The place is read into an IANA zone by a small model call and stored; a place that can't be resolved stores **nothing** and comes back `TIMEZONE_UNCLEAR`, so the shell can ask them to name it more plainly. Only a name the system's own timezone database carries is ever returned or stored, so an invented-but-well-formed zone is refused like a null one. |
| `GET /notifications` | Authed. Every channel that exists, mapped to whether this symbiot allows it — on by default until they turn it off — so the shell renders one toggle per channel already in the right position. |
| `POST /notifications` | Authed. Body `{ "channel": "<name>", "enabled": true\|false }`. A channel switched off is never fired for this symbiot again, whatever asks to reach them and however it is phrased — the dispatcher checks before every fan-out. The set of real channels lives in code, not duplicated here as a rule, so an unknown slug writes nothing and is a clean no-op. Either way the reply carries the full current state, so the shell re-renders from one source. |
| `GET /observe/echoes` | Authed. The `/observe` hub's echoes card: the symbiot's recent machine utterances scored for semantic redundancy, each tagged with the mechanism that raised it (a fast reply, a deep follow-up, a note) and the human line it answered, with the near-duplicates grouped into clusters. **Read-only** — it reports what the machine has already said and touches no path the loop runs on, so opening it can never change how the machine behaves. `scored` comes back `false` when the embedder was unreachable: the lens degrades to the plain mirror rather than erroring. `held_back` rides alongside — how many deep follow-ups the echo guard stopped before delivery, so the card reports the muzzle's successes and not only its failures. |
| `GET /observe/models` | Authed. The hub's second card, also read-only: which model actually answered the last calls that produced words for the symbiot — the fast reply and the deep follow-up. Every generative call resolves a *role* to a model and then walks the fallback ladder, so the model that answers is often not the one the role names, and the reply never says which one it was; the card sets the two names side by side, so a run of replies quietly composed by a humbler rung is legible instead of reading as the machine having a bad day. Each call carries its provider, the length of the raw answer (the cheapest tell for a model degenerating against its own output ceiling), and a local-time label. Box-level rather than per-symbiot, deliberately: which model served a call is a property of the machine and the clouds it can reach, exactly like the catalog and role assignments read through `/models`, and no row carries anyone's content. |
| `GET /observe/reminders` | Authed. The hub's third card, also read-only: the most recently scheduled reminders, each paired with the line that triggered it — so an over-eager schedule, set against a message that only mentioned a task, is visible. The **declines** ride alongside: the times the machine refused to set a reminder it judged already held. That is the one decision it takes on its own judgment rather than on the symbiot's words, and it writes no reminder by design, so without this the card would be blind to exactly the judgment most worth watching. |
...
| `GET /reminders` | Authed (`Authorization: Bearer <token>`). Reports the symbiot's standing reminder set in `data.reminders` — the live ones soonest first, then the last few that fired or were called off — each with its real id, its line, its `fire_at` already rendered on the symbiot's own clock, and a `state` of `pending`/`fired`/`cancelled`. The read the whole reminder lifecycle stands on: until it existed nothing could look at what the machine was holding. Unauthed → `{ "msg": "not authenticated", "data": { "authed": false } }`. |
| `POST /reminders`<br>`POST /reminders/cancel`<br>`POST /reminders/update` | Authed. The three writes over that set — add one typed straight in, call one off, or move its time and/or reword it (see the `Reminder*Request` DTOs in [`dtos.py`](./core/dtos.py)). Times are parsed **deterministically, kernel-side** against a small closed grammar (`+45m`, `20:05`, `2026-07-14 20:05`); anything outside it is refused rather than guessed at, because this path deliberately spends no model call. Every write is scoped in the statement itself — the row is yours, unfired, uncancelled — so one that fired in the meantime is refused rather than rewritten. A change that lands returns the full fresh set (`{ "msg": "reminders", … }`); a refusal returns `{ "msg": "that reminder change didn't take", "data": { …, "reason": "…" } }` with the set unchanged. Cancelling stamps `cancelled_at`; it never deletes. |
...
- **The role assignments** — which model, out of that catalog, does each generative job: `reply` (the composed answer), `rerank` (the router's classifications), `mint` (coining a new ontology type's definition), `enrich` (the follow-up gate), `tool_decision` (which tool a message asks for, and its arguments), `tool_confirm` (saying back what the tool did, in the symbiot's voice), `tool_observation_judge` (which of the rows a tool's observe hook found is the one), and `conversation_compress`. Each role points at one catalog model. The defaults group into three tiers, and the Gemini models **are** the primaries a role resolves to — the ones that answer in the steady state, with Scaleway and Mistral sitting beneath them as the outage path (see the ladder below). The flagship `gemini-3.1-pro-preview` takes the reply, the enrichment gate, and the conversation fold, where the words matter most; the mid `gemini-3.6-flash` takes the tool decision, the tool confirmation, and minting — competent language and structured extraction; the small `gemini-3.5-flash-lite` takes the two bounded picks, the router's re-rank and the observe-hook judgment.
...
When a role points at a **cloud** model, the call goes through a **fallback ladder**, tried per request ([`services/adapters/llm.py`](./services/adapters/llm.py)). A role pointing at a Gemini model — the default, and the tier's stable identity — gets the whole chain:

1. **Google (Gemini)** (top rung), reached on the **Agent Platform** through the `google-genai` client, authenticated by **ADC** (never an API key). This is the rung the roles resolve to and the one that answers in the steady state — the whole switch exists because the Agent Platform caches the large repeated prompt prefix these calls front-load, billing it once where the cheaper-per-token clouds re-bill it every turn. It is **per tier**, and the three primaries are genuinely distinct, each tier its own model: `gemini-3.1-pro-preview`, `gemini-3.6-flash`, `gemini-3.5-flash-lite`. Named by `FLAGSHIP_MODEL`, `MID_MODEL`, and `SMALL_MODEL`, the same three the role defaults are built from. Wired only when `GOOGLE_CLOUD_PROJECT` is set — leave it empty and this rung raises at once, so the ladder starts at Scaleway, exactly as before it existed.
2. **Scaleway**, reached through the OpenAI-compatible client Scaleway advertises. **Per tier**, keyed by the Gemini primary that fell through: the flagship falls to `glm-5.2`, and the two cheaper tiers share `gpt-oss-120b` — the split that is real in the Gemini rung above is not in this one. Named by `SCALEWAY_FLAGSHIP_MODEL`, `SCALEWAY_MID_MODEL`, and `SCALEWAY_SMALL_MODEL`.
3. **Mistral**, reached at Mistral's *own* web API through the official `mistralai` client — deliberately not Scaleway's Mistral, since the point is surviving Scaleway being down. **Per tier** in the same way, keyed by the Scaleway rung it catches: `glm-5.2` falls to `mistral-large-latest`, and `gpt-oss-120b` to `ministral-8b-latest`, each chosen so its window covers the rung above it. Named by `MISTRAL_FLAGSHIP_MODEL`, `MISTRAL_MID_MODEL`, and `MISTRAL_SMALL_MODEL`.
...
A cloud call tries the top rung and falls to the next only on an *outage-class* failure (transport error, timeout, 5xx, 429, an unresolvable ADC credential); a 4xx (a bad request, a bad key) surfaces at once rather than being masked. Where a role points decides how much of the ladder it gets: a Gemini model is the full four rungs above, a **Scaleway** model drops the Google rung for that role alone — the deliberate rollback — and keeps Mistral and the local floor beneath it, and a **Mistral** model is a **bare call** with nothing beneath it at all. When a role points at an **Ollama** model, there is no ladder either — the call goes straight to your local Ollama and never touches the cloud. So **pointing every role at an Ollama model, via `/models`, is what makes generation fully local** (see below).

**`IS_LOCAL=1` is the one-flag shortcut to all of that.** Set it and every generative call skips the ladder entirely and goes straight to your Ollama — no matter what provider the roles resolve to. Because the roles still resolve to *cloud* ids (a name like `gemini-3.6-flash` Ollama can't serve, so asking it to would 404), the call is substituted to the local floor `GENERATIVE_LOCAL_FALLBACK_MODEL` (`qwen3.5:4b`), unless a role already points at an Ollama model, in which case that one is honoured. It touches generation only — embedding was always local, and login delivery is still governed separately by the Gmail vars. So a box that wants generation local without reassigning eight roles by hand sets `IS_LOCAL=1` and pulls the one local model. **That model must actually be pulled** — `ollama pull qwen3.5:4b` (or whatever `GENERATIVE_LOCAL_FALLBACK_MODEL` names) — or the call 404s with `model '…' not found`, exactly as a cloud id would.

> **Heads-up on the generative calls:** thinking is off on every one — on Google through `thinking_level` held at each model's floor (`MINIMAL` for the Flash pair, `LOW` for the Pro, since a 3.x model can't turn reasoning off at all, only down), on Scaleway through `reasoning_effort`, set per model (`"none"` where the model accepts it, like `glm-5.2`; `"low"` for `gpt-oss`, whose floor is low but which emits no reasoning trace there), and on the Ollama tier with `think: false`. Structured output (the router's typed JSON) goes through each SDK's structured-output mechanism — Scaleway's and Mistral's `parse` helpers, Google's `response_schema`, Ollama's `format` — each binding the decoder to the caller's Pydantic schema; temperature is 0 on every call, the router's scored judgments and the spoken reply alike. These live in how [`services/adapters/llm.py`](./services/adapters/llm.py) calls each provider, not in the schema.
...
That's a fully-local symbiot — `IS_LOCAL=1` forces every generative call onto your Ollama, so you don't reassign a single role. (Prefer to keep some roles on the cloud and localise the rest? Leave `IS_LOCAL=0` and instead **`/models` → `assign` every role** — `reply`, `rerank`, `mint`, `enrich`, `tool_decision`, `tool_confirm`, `tool_observation_judge`, `conversation_compress` — **to `qwen3.5:4b`**; until you do, generation still points at the Gemini primaries and, unwired, falls to Scaleway and fails with a 401.) The rest of this section explains *why* each step is what it is — read on only if something above didn't behave.
...
5. **(Only if you left `IS_LOCAL=0`.) Point the generative roles at your local model — the fine-grained switch for generation.** With `IS_LOCAL=1` set in step 2, generation is already forced local and you can skip this step; do it instead when you want to split the work role-by-role (some local, some cloud), which the master switch can't express. Still in the shell, run **`/models`** (authed-only; you must be logged in). It opens on the current catalog and assignments, then loops on a prompt:
   ...
   - `assign` → point each role at your local model: assign `reply`, then `rerank`, `mint`, `enrich`, `tool_decision`, `tool_confirm`, `tool_observation_judge`, `conversation_compress`. Assigning `qwen3.5:4b` (already a built-in) needs no `add` first.
   - blank line → done.
...
| `FLAGSHIP_MODEL`, `MID_MODEL`, `SMALL_MODEL` | Override; the three tier primaries every role default is built from, and the top rung of the ladder. Default `gemini-3.1-pro-preview` / `gemini-3.6-flash` / `gemini-3.5-flash-lite` — genuinely distinct, each tier its own model. |
| `REPLY_MODEL`, `RERANK_MODEL`, `MINT_MODEL`, `ENRICH_MODEL`, `TOOL_DECISION_MODEL`, `TOOL_CONFIRM_MODEL`, `TOOL_OBSERVATION_JUDGE_MODEL`, `CONVERSATION_COMPRESS_MODEL` | Override; the **seed defaults** for each generative role's assignment, each naming a tier rather than a model of its own. `reply`, `enrich`, and `conversation_compress` take the flagship; `tool_decision`, `tool_confirm`, and `mint` the mid tier; `rerank` and `tool_observation_judge` the small one. These are read *once*, to seed a role's row the first time the box boots with the `model_role` table empty; after that the assignment lives in the database and is changed with the **`/models` command**, not here (see [Models](#models-embedding-local-and-generation-cloud-with-a-local-fallback) and [Running fully local](#running-fully-local-no-cloud-api-no-gmail)). Setting one here only changes what a *fresh* box seeds; to change a running box, use `/models`. |
| `SCALEWAY_FLAGSHIP_MODEL`, `SCALEWAY_MID_MODEL`, `SCALEWAY_SMALL_MODEL`, `MISTRAL_FLAGSHIP_MODEL`, `MISTRAL_MID_MODEL`, `MISTRAL_SMALL_MODEL`, `GENERATIVE_LOCAL_FALLBACK_MODEL` | Override; the rungs a call falls through to beneath the Gemini primary, each per tier. Scaleway catches first — flagship `glm-5.2`, the two cheaper tiers sharing `gpt-oss-120b` — then Mistral catches that, flagship `mistral-large-latest` and the cheaper rungs `ministral-8b-latest`, then the single local floor `qwen3.5:4b` beneath them all. Unlike the role assignments above, these are ladder mechanics, not `/models`-managed. |
